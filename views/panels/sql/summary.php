    ...
    <table data-ydtb-data-table>
        ...
        <?php foreach ($summary as $id => $entry): ?>
            <tr class="<?php echo($entry[1] > $this->countLimit || ($entry[4] / $entry[1] > $this->timeLimit) ? ' warning' : '') ?>">
                ...
                <?php $queryTimeColors = $this->queryTimeColor($entry[3]) ?>
                <td data-ydtb-data-type="number"
                    style="color:<?php echo isset($queryTimeColors['textColor']) ? $queryTimeColors['textColor'] : CLogger::LEVEL_ERROR; ?>;
                        background-color:<?php echo isset($queryTimeColors['backgroundColor']) ? $queryTimeColors['backgroundColor'] : CLogger::LEVEL_WARNING; ?>;">
                    <?php echo sprintf('%0.6F', $entry[3]); ?>
                </td>
            </tr>
        <?php endforeach; ?>
        </tbody>
