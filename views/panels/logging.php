    ...
    <div>
        <table data-ydtb-data-table>
            ...
            <tr>
                ...
                <th>Duration<br/>
                    <?php foreach ($this->colorsDuration as $item) : ?>
                        <span
                            style="background-color:<?php echo $item['backgroundColor']; ?>; width:9px; height:9px; display:block; float:left; margin:1px;"></span>
                    <?php endforeach; ?>
                </th>
            </tr>
            </thead>
            ...
            <?php foreach ($logs as $id => $entry): ?>
                <?php $r = $this->diffTime($entry[3], $old);
                $old = $entry[3]; ?>
                <tr>
                    ...
                    <td data-ydtb-data-type="number">
                        <?php echo date('H:i:s.', $entry[3]) .
                            sprintf('%06d', (int)(($entry[3] - (int)$entry[3]) * 1000000)); ?></td>
                    <td style="color:<?php echo isset($r['textColor']) ? $r['textColor'] : CLogger::LEVEL_ERROR; ?>;
                        background-color:<?php echo isset($r['backgroundColor']) ? $r['backgroundColor'] : CLogger::LEVEL_WARNING; ?>;">
                        <?php echo $r['duration']; ?>
                    </td>
                </tr>
            <?php endforeach; ?>
